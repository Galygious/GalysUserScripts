...
(function () {
    ...
    const clientInfoExtractorTool = {
        ...
        render(parentContainer) {
            let clientDataCache = []; // Cache for the fetched client data
            ...
            Object.assign(copyCsvButton.style, {
                padding: '8px 12px',
            ...
            copyCsvButton.textContent = 'Copy as CSV';

            copyCsvButton.addEventListener('click', () => {
                const csvHeader = '"Client Name","Client Email","Booked On"\n';
                const csvRows = clientDataCache.map(client =>
                    `"${client.name}","${client.email}","${client.bookedOn}"`
                ).join('\n');
            ...
            statusElement.style.maxHeight = '320px'; // Approx 10 rows + header
            ...
            getInfoButton.addEventListener('click', async () => {
                ...
                // Hide CSV button and clear cache
                copyCsvButton.style.display = 'none';
                clientDataCache = [];
                ...
                try {
                    ...
                    clientDataCache = clientData; // Cache the sorted data
                    ...
                    copyCsvButton.style.display = 'block'; // Show the CSV button
            ...
            toolContainer.appendChild(copyCsvButton);
    ...
    async function getClientInfo(startDate, endDate) {
        ...
        while(keepFetching) {
            ...
            debugLog(`Response data length for page ${page}: ${data.length} characters`);
            ...
            if (eventDivs.length > 0) {
                allEventDivs.push(...eventDivs);
            }

            if (eventDivs.length < numPerPage) {
                keepFetching = false;
            } else {
                page++;
            }
        ...
        debugLog(`Total events found: ${allEventDivs.length}`);

        const clientInfoPromises = allEventDivs.map(async (eventDiv) => {
